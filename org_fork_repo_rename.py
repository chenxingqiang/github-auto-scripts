...
for repo in repos:
    ...
    if response.status_code == 202:
        ...
        time.sleep(2)  # Waiting for the fork to complete

        # Rename the forked repo
        rename_url = f'https://api.github.com/repos/{new_org_name}/{repo["name"]}'
        rename_data = {'name': f'openmodels.{repo["name"]}'}
        rename_response = requests.patch(rename_url, headers=headers, json=rename_data)

        if rename_response.status_code == 200:
            print(f'Successfully renamed: {repo["name"]}')
        else:
            print(f'Failed to rename: {repo["name"]}. Status code: {rename_response.status_code}, Message: {rename_response.text}')
